...
async function getConfig() {
  const serviceAccountKeyDirect = core.getInput('gcp-service-account-key');
  const serviceAccountKeyFile = core.getInput('gcp-service-account-key-file');
  let serviceAccountKey = null;

  // Validate that both aren't set
  if (serviceAccountKeyDirect && serviceAccountKeyFile) {
    throw new Error('Cannot specify both gcp-service-account-key and gcp-service-account-key-file. Use one or the other.');
  }

  // Use direct key if provided (from secrets)
  if (serviceAccountKeyDirect) {
    core.info('Using service account key from input (secrets)');
    serviceAccountKey = serviceAccountKeyDirect;
  } else if (serviceAccountKeyFile) {
    // Otherwise, read service account key from file if provided
