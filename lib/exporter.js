...
function recordMetrics(meter, metrics, metricPrefix) {
  ...
  if (metrics.runner && metrics.runner.os) {
    ...
    const costHistogram = meter.createHistogram(`${metricPrefix}.job.estimated_cost`, {
    ...
    core.info(`Estimated cost for ${runnerDesc} runner: $${estimatedCost.toFixed(6)} (${(metrics.job.durationMs / 60000).toFixed(2)} minutes)`);
  }
