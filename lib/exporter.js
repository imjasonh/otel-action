...
function recordMetrics(meter, metrics, metricPrefix) {
  ...
  core.debug(`Job duration recorded with attributes: ${JSON.stringify(jobAttributes)}`);
  ...
  core.info('');
  core.info('=== Metrics Summary ===');
  core.info(`✓ Job duration: ${metricPrefix}.job.duration`);
  if (metrics.repository.sizeKB) {
    core.info(`✓ Repository size: ${metricPrefix}.repo.size`);
  }
  if (metrics.runner && metrics.runner.os) {
    core.info(`✓ Estimated cost: ${metricPrefix}.job.estimated_cost`);
  }
  if (metrics.artifacts && metrics.artifacts.count > 0) {
    core.info(`✓ Artifact sizes: ${metricPrefix}.artifact.size (${metrics.artifacts.count} artifacts)`);
  }
  const stepsWithDuration = metrics.steps.filter(s => s.durationMs > 0).length;
  core.info(`✓ Step durations: ${metricPrefix}.step.duration (${stepsWithDuration} steps)`);
  core.info('=====================');
  core.info('');
}
...
async function shutdown(meterProvider) {
  ...
  try {
    ...
    core.info('Triggering metric export via forceFlush()...');
    const flushResult = await meterProvider.forceFlush();
    core.debug(`forceFlush() result: ${JSON.stringify(flushResult)}`);
    core.info('✓ Metrics exported successfully');

    // Shutdown the provider
    core.info('Shutting down MeterProvider...');
    const shutdownResult = await meterProvider.shutdown();
    core.debug(`shutdown() result: ${JSON.stringify(shutdownResult)}`);
    ...
    core.error(`✗ Error during metric export/shutdown: ${errorMsg}`);
    core.error(`Stack trace: ${errorStack}`);
    core.error(`Error details: ${errorDetails}`);
    throw error;
  }
}
...
async function shutdownTracer(tracerProvider) {
  ...
  try {
    core.info('Triggering trace export via forceFlush()...');
    const flushResult = await tracerProvider.forceFlush();
    core.debug(`Trace forceFlush() result: ${JSON.stringify(flushResult)}`);
    core.info('✓ Traces exported successfully');

    core.info('Shutting down TracerProvider...');
    const shutdownResult = await tracerProvider.shutdown();
    core.debug(`Trace shutdown() result: ${JSON.stringify(shutdownResult)}`);
    ...
    core.error(`✗ Error during trace export/shutdown: ${errorMsg}`);
    core.error(`Stack trace: ${errorStack}`);
    core.error(`Error details: ${errorDetails}`);
    throw error;
  }
