...
const { MetricExporter: GCPMetricExporter } = require('@google-cloud/opentelemetry-cloud-monitoring-exporter');
const { TraceExporter: GCPTraceExporter } = require('@google-cloud/opentelemetry-cloud-trace-exporter');
...
/**
 * Creates and configures an OpenTelemetry MeterProvider
 * @param {Object} config - Configuration object
 * @returns {Object} MeterProvider and meters
 */
function createMeterProvider(config) {
  const useOtlp = !!config.otlpEndpoint;
  core.info(`Initializing OpenTelemetry MeterProvider with ${useOtlp ? 'OTLP' : 'Google Cloud Monitoring'} exporter`);
  ...
  if (useOtlp) {
    ...
    exporter = new GCPMetricExporter(exporterOptions);
  }
 ...
 * @param {Object} customAttributes - Custom user-provided attributes
 * @returns {Object} Base attributes object
 */
function buildBaseAttributes(metrics, customAttributes = {}) {
  ...
  // Add custom attributes (these override base attributes if there are conflicts)
  Object.assign(baseAttributes, customAttributes);
 ...
 * @param {Object} baseAttributes - Base attributes for all metrics (includes custom attributes)
 */
 ...
 * @param {Object} customAttributes - Custom user-provided attributes
 */
function recordMetrics(meter, metrics, metricPrefix, customAttributes = {}) {
  core.info('Recording metrics to OpenTelemetry');

  const baseAttributes = buildBaseAttributes(metrics, customAttributes);
...
async function shutdown(meterProvider) {
  ...
  try {
    ...
    core.error(`Error during export/shutdown: ${error?.message || JSON.stringify(error)}`);
    core.error(`Stack trace: ${error?.stack || 'N/A'}`);
    throw error;
  }
}
...
function createTracerProvider(config) {
  const useOtlp = !!config.otlpEndpoint;
  core.info(`Initializing OpenTelemetry TracerProvider with ${useOtlp ? 'OTLP' : 'Google Cloud Trace'} exporter`);
  ...
  if (useOtlp) {
    ...
    exporter = new GCPTraceExporter(exporterOptions);
  }
 ...
 */
function buildTraceBaseAttributes(metrics, customAttributes = {}) {
  const baseAttributes = {
    'workflow.name': metrics.workflow,
    'repository.owner': metrics.repository.owner,
    'repository.name': metrics.repository.repo,
 ...
 * @param {Object} metrics - Collected metrics
 * @param {Object} baseAttributes - Base attributes for the span (includes custom attributes)
 ...
 * @param {Object} tracer - OpenTelemetry tracer
 * @param {Object} metrics - Collected metrics
 * @param {Object} baseAttributes - Base attributes for spans (includes custom attributes)
...
function recordTraces(tracer, metrics, customAttributes = {}) {
  ...
  const baseAttributes = buildTraceBaseAttributes(metrics, customAttributes);
